...
```text
src/pitch.test.ts          # this repo: resolveKey, keyForSargam
src/score.test.ts          # this repo: parseScore
src/webmcp/embedTools.test.ts   # larger apps: split as in §3
```
...
- [ ] Extract shared types/helpers when the registrar grows (this repo keeps five tools in `src/webmcp.ts` until count exceeds ~15)
- [ ] Keep orchestrator readable (~120 lines when split; single file OK below that)
...
- [ ] Document tool list in `README.md` (this repo) or `tools.txt` / OpenAPI fragment (larger apps)
...
This project implements the patterns above with a single-file registrar (5 tools, under the split threshold):

| Artifact | Path |
|----------|------|
| Orchestrator + catalog + schemas | `src/webmcp.ts` (one file; split per §3 only when tool count grows) |
| Pure score validation | `src/score.ts` |
| Pitch/key resolution | `src/pitch.ts` |
| Unit tests | `src/pitch.test.ts`, `src/score.test.ts` |
| Eval fixtures | `evals/webmcp/fixtures/` |
| Evals README (CI vs manual) | `evals/webmcp/README.md` |
| Tool reference | `README.md` "WebMCP tools" section (no `tools.txt`) |
| Agent skills (rubrics) | `.agents/agent_tool_security/`, `.agents/webmcp-evals/`, `.agents/webmcp-page-tools/` |
...
**This repo** (harmonium — single-file registrar, five tools):

```text
Improve WebMCP on web-harmonium using .agents/webmcp-agent-guide.md.

Requirements:
- Feature-detect document.modelContext; no-op if missing
- Keep src/webmcp.ts unless tool count exceeds ~15; pure logic in src/score.ts and src/pitch.ts
- readOnlyHint on every tool; uniform { ok, tool, error?, ...data } execute returns
- npm test covers parseScore, resolveKey; evals/webmcp/fixtures/ has expectedCall JSON
- Do not touch backend MCP unless asked

Verify: npm run typecheck && npm test && npm run build
Manual: Chromium #enable-webmcp-testing; call get_state then play_note; stop must silence audio
```

**Other repos** (split registrar when tool count grows):

```text
Implement WebMCP using .agents/webmcp-agent-guide.md §3 layout.
...
- `evals/webmcp/README.md` — what runs in CI vs manual
